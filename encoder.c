...
#include "lteration3.h"
#include "lteration4.h"
#include "lteration5.h"
...
int main (int argc, char **argv) {
	...
	if (strcmp(argv[6], "EBR") == 0) {
		...
		if (tau!=1) {
			fprintf(stderr,  "tau must be a 1\n");
			exit(0);
		}
		tech = EBR;
	}
...
char **coding2;
coding2 = (char **)malloc(sizeof(char*)*p);
for(j = 0; j < p; j++) {
...
printf("\ncoding1--2------------------------------\n");	
			...
			for(j1=0;j1<k;j1++){
			shift5[j1]=finite_mod((i-pp1[i2][j1]) , p);}
...
timing_set(&q5);
char **fdataa;
fdataa=(char **)malloc(sizeof(char*)*p);
for(i=0;i<p;i++){
fdataa[i]=(char *)malloc(sizeof(char)*r*blocksize);}

if(r==3){
fdataa=lteration_3( coding2, k, r, p, blocksize,k,k+1,k+2);}

if(r==4){
fdataa=lteration_4( coding2, k, r, p, blocksize,k,k+1,k+2,k+3);}

if(r==5){
fdataa=lteration_5( coding2, k, r, p, blocksize,k,k+1,k+2,k+3,k+4);}
...
/* Write data and encoded data to k+m files */
		...
		lteration_time=timing_delta(&q5,&q6);
		sum_time=extra_bit_time+first_matrix_mul_time+lteration_time;
